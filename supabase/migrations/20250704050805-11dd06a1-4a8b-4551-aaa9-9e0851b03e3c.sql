-- Add analysis_id field to user_journey_complete table to properly link journeys to property analyses
ALTER TABLE public.user_journey_complete 
ADD COLUMN IF NOT EXISTS analysis_id UUID REFERENCES public.user_property_analyses(id);

-- Create index for better performance
CREATE INDEX IF NOT EXISTS idx_user_journey_complete_analysis_id 
ON public.user_journey_complete(analysis_id);

-- Update the update_journey_step function to handle analysis_id
...
-- Drop and recreate the get_user_dashboard_data function with analysis_id
