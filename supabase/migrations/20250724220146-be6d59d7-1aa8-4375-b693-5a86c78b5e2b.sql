...
-- Drop existing restrictive policies that prevent modifications
...
-- Create comprehensive RLS policies for enhanced_service_providers
CREATE POLICY "Public read access to enhanced service providers" ON enhanced_service_providers
FOR SELECT USING (true);

CREATE POLICY "Service role can manage enhanced service providers" ON enhanced_service_providers
FOR ALL USING (
  (auth.jwt() ->> 'role'::text) = 'service_role'::text
);

CREATE POLICY "Authenticated users can insert enhanced service providers" ON enhanced_service_providers
FOR INSERT WITH CHECK (
  auth.uid() IS NOT NULL
);

CREATE POLICY "Authenticated users can update enhanced service providers" ON enhanced_service_providers
FOR UPDATE USING (
  auth.uid() IS NOT NULL
);

CREATE POLICY "Authenticated users can delete enhanced service providers" ON enhanced_service_providers
FOR DELETE USING (
  auth.uid() IS NOT NULL
);
