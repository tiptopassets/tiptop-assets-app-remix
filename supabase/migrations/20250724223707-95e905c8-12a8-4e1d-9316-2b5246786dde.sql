...
-- Fix RLS policies for enhanced_service_providers table
-- First, drop all existing policies to start fresh
DROP POLICY IF EXISTS "Public read access to enhanced providers" ON enhanced_service_providers;
DROP POLICY IF EXISTS "Public read access to enhanced service providers" ON enhanced_service_providers;

-- Create new comprehensive RLS policies
CREATE POLICY "Anyone can read enhanced service providers" 
ON enhanced_service_providers FOR SELECT 
USING (true);

CREATE POLICY "Service role can manage enhanced service providers" 
ON enhanced_service_providers FOR ALL 
USING ((auth.jwt() ->> 'role'::text) = 'service_role'::text);

CREATE POLICY "Admins can manage enhanced service providers" 
ON enhanced_service_providers FOR ALL 
USING (EXISTS (
...
-- Remove duplicate Peerspace entries (keep the most recent one)
WITH duplicate_peerspace AS (
...
WHERE id IN (
  SELECT id FROM duplicate_peerspace WHERE rn > 1
);

-- Remove incorrect entries that shouldn't be there
DELETE FROM enhanced_service_providers 
WHERE name IN ('Giggster', 'Grass.io', 'Sniffspot');
...
-- Insert missing partners with proper data
...
('Airbnb Unit Rental', 'Rent out your property or spare rooms to travelers on Airbnb',
 ...
 800, 3000, 10, true, 'https://www.airbnb.com/host/homes'),
...
('Airbnb Experience', 'Create and host unique experiences for travelers in your area',
 ...
 200, 1500, 8, true, 'https://www.airbnb.com/host/experiences'),
...
('Airbnb Service', 'Offer services to Airbnb hosts and guests in your area',
 ...
 300, 2000, 7, true, 'https://www.airbnb.com/host/services'),
...
-- Update existing providers with missing logos
UPDATE enhanced_service_providers 
SET logo = CASE 
