...
-- First, add the missing RLS policies for enhanced_service_providers table
-- This will allow INSERT, UPDATE, and DELETE operations

-- Allow service role to manage all operations on enhanced_service_providers
CREATE POLICY "Service role can manage enhanced providers" ON enhanced_service_providers
FOR ALL USING (
  (auth.jwt() ->> 'role'::text) = 'service_role'::text
);

-- Allow authenticated users to insert new providers (for admin functionality)
CREATE POLICY "Authenticated users can insert providers" ON enhanced_service_providers
FOR INSERT WITH CHECK (
  auth.uid() IS NOT NULL
);

-- Allow authenticated users to update providers (for admin functionality)
CREATE POLICY "Authenticated users can update providers" ON enhanced_service_providers
FOR UPDATE USING (
  auth.uid() IS NOT NULL
);

-- Allow authenticated users to delete providers (for admin functionality)
CREATE POLICY "Authenticated users can delete providers" ON enhanced_service_providers
FOR DELETE USING (
  auth.uid() IS NOT NULL
);

-- Now clean up the existing data
