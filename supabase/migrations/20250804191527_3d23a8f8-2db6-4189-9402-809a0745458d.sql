...
-- Add basic RLS policies for new tables (user-scoped tables)
CREATE POLICY "Users can view their own property images" ON user_property_images FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own property images" ON user_property_images FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own property images" ON user_property_images FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can view their own journey data" ON user_journey_complete FOR SELECT USING (auth.uid() = user_id OR user_id IS NULL);
CREATE POLICY "Users can insert their own journey data" ON user_journey_complete FOR INSERT WITH CHECK (auth.uid() = user_id OR user_id IS NULL);
CREATE POLICY "Users can update their own journey data" ON user_journey_complete FOR UPDATE USING (auth.uid() = user_id OR user_id IS NULL);

CREATE POLICY "Users can view their own journey progress" ON user_journey_progress FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own journey progress" ON user_journey_progress FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own journey progress" ON user_journey_progress FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can view their own visitor sessions" ON visitor_sessions FOR SELECT USING (auth.uid() = user_id OR user_id IS NULL);
CREATE POLICY "Users can insert their own visitor sessions" ON visitor_sessions FOR INSERT WITH CHECK (auth.uid() = user_id OR user_id IS NULL);
CREATE POLICY "Users can update their own visitor sessions" ON visitor_sessions FOR UPDATE USING (auth.uid() = user_id OR user_id IS NULL);
...
CREATE POLICY "Users can view messages from their onboarding" ON onboarding_messages FOR SELECT USING (
  EXISTS (SELECT 1 FROM user_onboarding WHERE id = onboarding_id AND user_id = auth.uid())
);
CREATE POLICY "Users can insert messages to their onboarding" ON onboarding_messages FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM user_onboarding WHERE id = onboarding_id AND user_id = auth.uid())
);

CREATE POLICY "Users can view their own service selections" ON user_service_selections FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own service selections" ON user_service_selections FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own service selections" ON user_service_selections FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can view their own supplier connections" ON user_supplier_connections FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own supplier connections" ON user_supplier_connections FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own supplier connections" ON user_supplier_connections FOR UPDATE USING (auth.uid() = user_id);

-- Public read access for reference tables
CREATE POLICY "Public read access for provider supported assets" ON provider_supported_assets FOR SELECT USING (true);
CREATE POLICY "Public read access for provider setup requirements" ON provider_setup_requirements FOR SELECT USING (true);
CREATE POLICY "Public read access for available services" ON available_services FOR SELECT USING (true);
CREATE POLICY "Public read access for solar cache" ON solar_api_cache FOR SELECT USING (true);
CREATE POLICY "Public read access for enhanced solar cache" ON enhanced_solar_cache FOR SELECT USING (true);

-- Admin-only access for analytics and recommendations
CREATE POLICY "Admin access for journey analytics" ON journey_analytics FOR ALL USING (get_current_user_role() = 'admin');
CREATE POLICY "Public read access for partner recommendations" ON partner_recommendations FOR SELECT USING (true);
CREATE POLICY "Users can view their own integration progress" ON partner_integration_progress FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own integration progress" ON partner_integration_progress FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own integration progress" ON partner_integration_progress FOR UPDATE USING (auth.uid() = user_id);